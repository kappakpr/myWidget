...
nrow(train_valid)
predictors = as.matrix(train_valid[,1:10]);   classes = as.matrix(train_valid[,11])

ccAppModel2 = ksvm(predictors ,classes ,type="C-svc",kernel="vanilladot",C=Cval,scaled=TRUE);ccAppModel2 
a = colSums(predictors[ccAppModel2@SVindex,1:10] * ccAppModel2@coef[[1]]) #; a
a0 = - ccAppModel2@b; a0
print(a0)

#training + valid data check
cctrainvalidPrediction = predict(ccAppModel2 ,train_valid[,1:10]) ;cctrainvalidPrediction 
trainvalidsumval=sum(cctrainvalidPrediction == train_valid[,11]) / nrow(train_valid)
print(c(Cval, trainvalidsumval))

trainvalidpred<-predict(ccAppModel2,train_valid[,-11]); trainvalidpred
pred_table_trainvalid<-table(trainvalidpred,train_valid$R1);
print(pred_table_trainvalid)
pred_accuracy_trainvalid<-sum(diag(pred_table_trainvalid)/sum(pred_table_trainvalid)) ;pred_accuracy_trainvalid

#test data check 
cctestPrediction = predict(ccAppModel2 ,test[,1:10]) ;cctestPrediction 
testsumval=sum(cctestPrediction == test[,11]) / nrow(test)
print(c(Cval, testsumval))

testpred<-predict(ccAppModel2,test[,-11]); testpred
pred_table_test<-table(testpred,test$R1);
print(pred_table_test)
pred_accuracy_test<-sum(diag(pred_table_test)/sum(pred_table_test)) ;pred_accuracy_test
